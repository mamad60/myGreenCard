...
function saveToDB()
{
   ...
   // Setup Connection to the MySQL database
    ...
    // IF all goes well insert request date into the DB
    if (empty($output['error'])) {
        try {
            $sql = "UPDATE applicant SET requestDate = '" .time(). "' WHERE ID='" . $last_id . "'";
                        $conn->exec($sql);
            
                    } catch (PDOException $e) {
                        $output['error'][] = 'Databsase Error,' . 'Data Insersion failed(Request Date):' . $e->getMessage();
                    }
    ...
    if (empty($output['error'])) {
        ...
        $body = "<html>
                <body>
                <div dir='rtl'>سلام، <br/> ثبت نام جدید در سایت Mygreen Card\n</div>
                ID=$last_id<br/>
                Total Paid= '" . $price['TotalCharge'] . "'<br/>
                TransID='" . $_SESSION['TransId'] . "'<br/>
